...
CREATE TABLE template_scopes (
    ...
    data_type VARCHAR(20) DEFAULT 'boolean', -- boolean, string, number, enum
...
CREATE TABLE company_scope_responses (
    ...
    response_value JSONB, -- Can store boolean, string, or other data types
    confidence DECIMAL(3, 2) NOT NULL DEFAULT 1.0,
    source_document_type VARCHAR(50), -- 'privacy', 'terms', etc.
    source_url VARCHAR(2048),
    extraction_method VARCHAR(50), -- 'llm', 'manual', 'heuristic'
    reasoning TEXT,
...
CREATE INDEX idx_company_scope_responses_company ON company_scope_responses(company_id);
CREATE INDEX idx_company_scope_responses_scope ON company_scope_responses(template_scope_id);
